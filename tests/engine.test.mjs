...
test('Tutorial SVGs contain real nuclei, not unexpanded templates',()=>{for(const lang of ['it','en','fr']){const half=fs.readFileSync(`tutorial/${lang}/half-life.svg`,'utf8');assert.equal((half.match(/<circle/g)||[]).length,14);for(const name of ['half-life','zan'])assert(!fs.readFileSync(`tutorial/${lang}/${name}.svg`,'utf8').includes('join('));}});
